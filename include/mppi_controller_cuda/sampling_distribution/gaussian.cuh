...
public:
   ...
   * @param distribution_index - distribution index
  ...
  /**
   * @brief Update the distribution according to the weights of each sample
   ...
   * @param alpha - coeff to turn off the likelihood cost
   * cost)
   */
   ...
   * @param synchronize - whether or not to run cudaStreamSynchronize
   * @param regenerate_noise - if true draws a fresh curand N(0,1) batch and
   * caches it in cached_raw_noise_d_ before combining with the current mean/std_dev. If false, skips the curand
   * draw entirely and re-combines the previously cached raw noise with the current
   */
