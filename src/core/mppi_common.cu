...
#include "mppi_controller_cuda/utils/cuda_math_utils.cuh"
#include "../dynamics/dynamics.cu"
#include "../dynamics/motion_model.cu"
#include "../costs/cost.cu"
#include "../utils/texture_helpers/texture_helper.cu"
#include "../utils/texture_helpers/two_d_texture_helper.cu"
#include "../costs/motion_model_cost.cu"
#include "../sampling_distribution/gaussian.cu"
#include "../sampling_distribution/colored_noise.cu"

namespace mppi
{
namespace sampling_distributions
{
template class GaussianDistributionImpl<GaussianParams, MotionModel::DYN_PARAMS_T>;
template class ColoredNoiseDistributionImpl<ColoredNoiseParams, MotionModel::DYN_PARAMS_T>;
template class GaussianDistributionImpl<ColoredNoiseParams, MotionModel::DYN_PARAMS_T,
    ColoredNoiseDistributionImpl<ColoredNoiseParams, MotionModel::DYN_PARAMS_T>>;
}  // namespace sampling_distributions
}  // namespace mppi
...
__global__ void rolloutKernel(DYN_T* __restrict__ dynamics, SAMPLING_T* __restrict__ sampling,
                              ...
                              float* __restrict__ trajectory_costs_d, int* __restrict__ crash_status_d)
{
  ...
  __syncthreads();
  MotionModelParams accel_limits = dynamics->getParams();
  float last_vx = accel_limits.current_vx;
  float last_vy = accel_limits.current_vy;
  float last_wz = accel_limits.current_wz;
  float last_traj_dist = 0.0f;
  for (int t = 0; t < num_timesteps; t++)
  {
    ...
    const float max_delta_vx = dt * accel_limits.ax_max;
    const float min_delta_vx = dt * accel_limits.ax_min;
    const float max_delta_vy = dt * accel_limits.ay_max;
    const float max_delta_wz = dt * accel_limits.az_max;
    u[C_IND_CLASS(MotionModelParams, VX)] =
        fminf(fmaxf(u[C_IND_CLASS(MotionModelParams, VX)], last_vx + min_delta_vx), last_vx + max_delta_vx);
    u[C_IND_CLASS(MotionModelParams, VY)] =
        fminf(fmaxf(u[C_IND_CLASS(MotionModelParams, VY)], last_vy - max_delta_vy), last_vy + max_delta_vy);
    u[C_IND_CLASS(MotionModelParams, WZ)] =
        fminf(fmaxf(u[C_IND_CLASS(MotionModelParams, WZ)], last_wz - max_delta_wz), last_wz + max_delta_wz);

    dynamics->enforceConstraints(x, u);
    last_vx = u[C_IND_CLASS(MotionModelParams, VX)];
    last_vy = u[C_IND_CLASS(MotionModelParams, VY)];
    last_wz = u[C_IND_CLASS(MotionModelParams, WZ)];
#ifdef USE_CUDA_BARRIERS_ROLLOUT
    bar->arrive_and_wait();
#else
    __syncthreads();
    ...
    {
      const float traj_dx =
          x_next[S_IND_CLASS(MotionModelParams, X)] - x[S_IND_CLASS(MotionModelParams, X)];
      const float traj_dy =
          x_next[S_IND_CLASS(MotionModelParams, Y)] - x[S_IND_CLASS(MotionModelParams, Y)];
      last_traj_dist += sqrtf(traj_dx * traj_dx + traj_dy * traj_dy);
      y[O_IND_CLASS(MotionModelParams, TRAJ_DIST)] = last_traj_dist;
    }
    running_cost[0] +=
  ...
  if (thread_idy == 0)
  {
    crash_status_d[global_idx + num_rollouts * thread_idz] = crash_status[0];
  }
                                  ...
                                  int* __restrict__ crash_status_d)
{
  ...
  if (thread_idx == blockDim.x - 1 && thread_idy == 0)
  {
    int any_crash = 0;
    for (int i = 0; i < blockDim.x; i++)
    {
      if (crash_status_shared[thread_idz * blockDim.x + i])
      {
        any_crash = 1;
        break;
      }
    }
    crash_status_d[global_idx + num_rollouts * thread_idz] = any_crash;
  }
                                   ...
                                   float running_cost, float* theta_c, float* cost_rollouts_device)
{
  ...
  if (threadIdx.y == 0 && global_idx < num_rollouts)
  {
    cost_rollouts_device[global_idx + num_rollouts * threadIdx.z] =
        running_cost + costs->terminalCost(output, theta_c);
  }
}
...
__global__ void resetFurthestReachedKernel(MotionModelCost* __restrict__ costs_d)
{
  costs_d->resetFurthestReached();
}

__global__ void computeFurthestReachedKernel(MotionModelCost* __restrict__ costs_d, const float* __restrict__ y_d,
                                              int num_timesteps, int num_rollouts)
{
  ...
  int offset = (rollout * num_timesteps + t) * MotionModelCost::OUTPUT_DIM;
  float end_x = y_d[offset + O_IND_CLASS(MotionModelParams, X)];
  float end_y = y_d[offset + O_IND_CLASS(MotionModelParams, Y)];
  costs_d->updateFurthestReachedFromEndpoint(end_x, end_y);
}
...
void launchSplitRolloutKernel(DYN_T* __restrict__ dynamics, COST_T* __restrict__ costs,
                              ...
                              float* __restrict__ y_d, float* __restrict__ trajectory_costs,
                              int* __restrict__ crash_status_d, dim3 dimDynBlock, dim3 dimCostBlock,
                              cudaStream_t stream, bool synchronize)
{
  ...
  resetFurthestReachedKernel<<<1, 1, 0, stream>>>(costs->cost_d_);
  const int reduce_grid = math::int_ceil(num_rollouts, 256);
  computeFurthestReachedKernel<<<reduce_grid, 256, 0, stream>>>(costs->cost_d_, y_d, num_timesteps, num_rollouts);
  HANDLE_ERROR(cudaGetLastError());
  // Run Costs
...
void launchRolloutKernel(DYN_T* __restrict__ dynamics, COST_T* __restrict__ costs, SAMPLING_T* __restrict__ sampling,
                         ...
                         int* __restrict__ crash_status_d, dim3 dimBlock, cudaStream_t stream, bool synchronize)
{
  ...
  rolloutKernel<DYN_T, COST_T, SAMPLING_T><<<dimGrid, dimBlock, shared_mem_size, stream>>>(
      ...
      alpha, trajectory_costs, crash_status_d);
  HANDLE_ERROR(cudaGetLastError());
  if (synchronize)
  {
    HANDLE_ERROR(cudaStreamSynchronize(stream));
...
using SAMPLING_T_COLORED = mppi::sampling_distributions::ColoredNoiseDistribution<MotionModel::DYN_PARAMS_T>;

#define INSTANTIATE_SAMPLING_KERNELS(SAMPLING_T)                                                                     \
  template void launchSplitRolloutKernel<MotionModel, MotionModelCost, SAMPLING_T>(                                 \
      MotionModel* __restrict__, MotionModelCost* __restrict__, SAMPLING_T* __restrict__, float, const int,         \
      const int, float, float, float* __restrict__, float* __restrict__, float* __restrict__, int* __restrict__,    \
      dim3, dim3, cudaStream_t, bool);                                                                               \
                                                                                                                       \
  template void launchRolloutKernel<MotionModel, MotionModelCost, SAMPLING_T>(                                      \
      MotionModel* __restrict__, MotionModelCost* __restrict__, SAMPLING_T* __restrict__, float, const int,         \
      const int, float, float, float* __restrict__, float* __restrict__, int* __restrict__, dim3, cudaStream_t,     \
      bool);                                                                                                         \
                                                                                                                       \
  template void launchVisualizeCostKernel<MotionModelCost, SAMPLING_T>(                                             \
      MotionModelCost* __restrict__, SAMPLING_T* __restrict__, float, const int, const int, float, float,           \
      float* __restrict__, int* __restrict__, float* __restrict__, dim3, cudaStream_t, bool);                       \
                                                                                                                       \
  template void launchVisualizeKernel<MotionModel, MotionModelCost, SAMPLING_T>(                                    \
      MotionModel* __restrict__, MotionModelCost* __restrict__, SAMPLING_T* __restrict__, float, const int,         \
      const int, float, float, float* __restrict__, float* __restrict__, float* __restrict__, int* __restrict__,    \
      dim3, cudaStream_t, bool);                                                                                     \
                                                                                                                       \
  template unsigned calcRolloutDynamicsKernelSharedMemSize<MotionModel, SAMPLING_T>(const MotionModel*,             \
                                                                                     const SAMPLING_T*, dim3&);       \
                                                                                                                       \
  template unsigned calcRolloutCostKernelSharedMemSize<MotionModelCost, SAMPLING_T>(const MotionModelCost*,         \
                                                                                     const SAMPLING_T*, dim3&);       \
                                                                                                                       \
  template unsigned calcRolloutCombinedKernelSharedMemSize<MotionModel, MotionModelCost, SAMPLING_T>(               \
      const MotionModel*, const MotionModelCost*, const SAMPLING_T*, dim3&);                                        \
                                                                                                                       \
  template unsigned calcVisualizeKernelSharedMemSize<MotionModel, MotionModelCost, SAMPLING_T>(                     \
      const MotionModel*, const MotionModelCost*, const SAMPLING_T*, const int&, dim3&);

INSTANTIATE_SAMPLING_KERNELS(SAMPLING_T_COLORED)
#undef INSTANTIATE_SAMPLING_KERNELS

// Used directly from GaussianDistributionImpl::updateDistributionParamsImpl (gaussian.cu)
// with CONTROL_DIM = MotionModel::CONTROL_DIM.
template void launchWeightedReductionKernel<MotionModel::CONTROL_DIM>(const float* __restrict__,
